...
#undef IMM_MUTATE_DECLARE
 ...
 private:
  ...
  template<typename T>
  ExprPtr get_cached_expr(const std::shared_ptr<T>& ptr);
  template<typename T>
  StmtPtr get_cached_stmt(const std::shared_ptr<T>& ptr);
  template<typename T>
  ExprPtr set_cached_expr(const std::shared_ptr<T>& ptr, ExprPtr v);
  template<typename T>
  StmtPtr set_cached_stmt(const std::shared_ptr<T>& ptr, StmtPtr v);
};
